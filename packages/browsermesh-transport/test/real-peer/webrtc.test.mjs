...
describeIfReal('WebRTC against real peers', () => {
  ...
  async function connect({ alice, bob }) {
    /*
     * No retry. There was one until node-datachannel 0.33.4, which is the
     * first release whose prebuilt binary is built against libdatachannel
     * v0.24.5 — the release carrying #1584, the unsynchronised BIO write that
     * made roughly 1 handshake in 130 report a fatal I/O error that never
     * happened. browsermesh#26 and CLAUDE.md carry the measurement.
     *
     * The deadline stays generous rather than tight: a success takes about
     * 700ms, so 15s fails nothing that is merely slow, and a real failure is
     * terminal (`connectionState` goes to `failed`) rather than slow.
     */
  ...
  it('a malformed remote ICE candidate is rejected without killing the process', async () => {
    ...
    try {
       ...
       * handleAnswer(). That made it the one test in this file exposed to the
       * upstream DTLS flake while every other test rode connect()'s retry —
       * not a weaker test than the others, the only honest one about a bug
       * they were all hiding. Both retries are gone as of 0.33.4.
       */
  ...
  it('reports a connection the stack gave up on, instead of waiting forever', async () => {
    ...
    try {
      ...
      //
      // Wait for a TERMINAL state, and accept either one, because which one
      // arrives is a property of the stack rather than of this code:
      //
      //   - a browser reports `failed` and never `closed` (measured against
      //     native WebRTC: failed at ~51ms, still failed 20s later). Per spec
      //     connectionState `closed` means the LOCAL object was closed, so a
      //     remote DTLS failure cannot produce it.
      //   - libdatachannel usually sends `closed` as well, which is why this
      //     test used to pass while asserting `closed` -- and flaked at ~9%
      //     on the runs where it behaved like a browser.
      //
      // What must hold on every stack is that the connection stops claiming
      // to be connecting and `isOpen` goes false.
      await waitFor(
        () => peers.alice.state === 'failed' || peers.alice.state === 'closed',
        8_000,
        'the connection to reach a terminal state',
      )

      assert.notEqual(peers.alice.state, 'connecting', 'a dead connection does not stay "connecting"')
