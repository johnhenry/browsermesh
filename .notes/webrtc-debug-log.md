# Internal notes: WebRTC flake debugging session log

> **This is a session log, not doc-conventions guidance.** It was previously
> checked in as the repo's root `CLAUDE.md`, which made it look like agent
> instructions when it's actually a debugging record of one specific
> `node-datachannel` handshake-flake investigation. Kept here for the
> historical record and because the "pending upstream revert" section still
> has a live action item. See `../CLAUDE.md` / `../AGENTS.md` for actual repo
> conventions.
